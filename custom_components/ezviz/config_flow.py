...
class EzvizConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def _validate_and_create_camera_rtsp(self, data: dict) -> ConfigFlowResult:
        ...
        return self.async_create_entry(
            ...
            data={
                ...
                # Optional verification code used to decrypt alarm images.
                CONF_ENC_KEY: data.get(CONF_ENC_KEY) or None,
    ...
    async def async_step_reconfigure(
        ...
        """Handle reconfiguration of a camera RTSP entry.

        Lets the user update the camera username/password and add or change
        the verification code used to decrypt encrypted motion images, without
        having to delete and re-add the camera.
        """
        entry = self._get_reconfigure_entry()

        # Only per-camera RTSP entries carry credentials worth reconfiguring.
        # The cloud account password is handled by the reauth flow instead.
        ...
        if user_input is not None:
            return self.async_update_reload_and_abort(
                ...
                data_updates={
                    ...
                    CONF_ENC_KEY: user_input.get(CONF_ENC_KEY) or None,
                },
            )
        ...
        data_schema = vol.Schema(
            {
                vol.Required(
                    CONF_USERNAME,
                    default=entry.data.get(CONF_USERNAME, DEFAULT_CAMERA_USERNAME),
                ): str,
                vol.Required(CONF_PASSWORD): str,
                vol.Optional(
                    CONF_ENC_KEY,
                    description={"suggested_value": entry.data.get(CONF_ENC_KEY)},
                ): str,
            }
        )
        ...
        return self.async_show_form(
            step_id="reconfigure",
            data_schema=data_schema,
            description_placeholders={ATTR_SERIAL: entry.title},
        )
