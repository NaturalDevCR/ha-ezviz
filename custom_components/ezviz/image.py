...
from .const import CONF_ENC_KEY, DOMAIN
...
class EzvizLastMotion(EzvizEntity, ImageEntity):
    ...
    def __init__(
        ...
        # EZVIZ encrypts alarm images with the device verification code, which
        # may differ from the RTSP password. Prefer the dedicated key and fall
        # back to the RTSP password for backwards compatibility.
        self.alarm_image_password = (
            camera.data.get(CONF_ENC_KEY) or camera.data.get(CONF_PASSWORD)
