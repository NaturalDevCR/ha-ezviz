...
class EzvizSensor(EzvizEntity, SensorEntity):
    ...
    def native_value(self) -> int | str | None:
        ...
        Home Assistant truncates states longer than 255 characters to
        ``unknown``. EZVIZ alarm picture URLs (signed, with many query
        params) routinely exceed this, so the full value is exposed via
        ``extra_state_attributes`` and the state is capped instead.
        """
    ...
    def extra_state_attributes(self) -> dict[str, str] | None:
        """Expose the full value when it is too long for the state."""
        value = self.data[self._sensor_name]
        if isinstance(value, str) and len(value) > 255:
